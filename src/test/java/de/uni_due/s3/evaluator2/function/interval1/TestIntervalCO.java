...
public class TestIntervalCO extends TestFunctionAbstract{

	private final Function func = new IntervalCO(); 
	@Test
	public void testIntervallCC() throws EvaluatorException, OpenMathException {
		...
		assertEquals(OMSymbol.INTERVAL1_INTERVALCO,result.getOmel().get(0));
		assertEquals(OMCreator.createOMI(1),result.getOmel().get(1));
		assertEquals(OMCreator.createOMI(3),result.getOmel().get(2));
	}
	
	@Test
	public void testIntervalCOLatexSyntax() throws EvaluatorException, OpenMathException {
		List<Object> args = new ArrayList<Object>(2);
		args.add(OMCreator.createOMI(1));
		args.add(OMCreator.createOMI(3));
