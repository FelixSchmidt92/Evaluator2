 ...
 * 
 * For now (09.06.17) it can only traverse OMOBJ,OMA,OMF,OMI,OMS,OMSTR and OMV.
 * All other OpenMath objects are not supported.
 * 
 * @author frichtscheid
 *
 */
public class OMExecutor {
	...
	/**
	 * Visits a OMOBJ object and return its child object. The kind of child is
	 * determined by the OMOBJ object. At the moment OMA,OMF,OMI,OMS,OMSTR and OMV
	 * are those children which will be returned.
	 * 
	 * @param omobj
	 ...
	 */
	public static OMOBJ execute(OMOBJ omobj) throws EvaluatorException, OpenMathException {
	...
	/**
	 * Visits a OpenMath application object (OMA). Although a OMA can have different
	 * kinds of children as its first child, like OMV or OMS, we assume that its
	 * first child is a OMS, because without a function their is no need for a OMV?
	 * Example: <OMA> <OMS cd="arith1" name="plus" /> ... here are the arguments for
	 * the function </OMA> After the OMS any amount of OMF,OMI,OMS,OMSTR,OMV or
	 * another OMA can follow. If the function (defined by the OMS) allows that
	 * children of the type OMA can be visited, they will be visited and the result
	 * of that will be used as argument instead of the OMA itself. All children of a
	 * OMA (without the first child) will be passed to the function as arguments and
